<template>
  <div class="page-hero" v-if="model">
    <div class="topbar bg-black py-2 px-3 d-flex ai-center text-white">
      ...
      <router-link class="text-decoration text-white" to="/">
        <div>更多英雄 &gt;</div></router-link
      >
    </div>
    ...
    <div>
      <div class="bg-white px-3">
        <div class="nav d-flex jc-around pt-3 pb-2 border-bottom">
          <div class="nav-item " @click="onSwiper.slideTo(0)" :class="{active : !isShow}">
            <div class="nav-link">英雄初识</div>
          </div>
          <div class="nav-item" @click="onSwiper.slideTo(1)" :class="{active: isShow}">
            <div class="nav-link">进阶攻略</div>
          </div>
        </div>
      </div>
      <swiper @swiper=" (swiper)=> onSwiper = swiper"  @slideChange="isShow = !isShow" >
        <swiper-slide>
          <div>
            <div class="p-3 bg-white border-bottom">
              <div class="d-flex">
                <router-link to="/" class="btn btn-lg flex-1 text-decoration">
                  <i class="iconfont icon-menu1"></i>
                  英雄介绍视频
                </router-link>
                <router-link
                  to="/"
                  class="btn btn-lg flex-1 ml-2 text-decoration"
                >
                  <i class="iconfont icon-menu1"></i>
                  一图识英雄
                </router-link>
...
const currentSkill = computed(() => {
  return model.value.skills[currentSkillIndex.value];
});
onMounted(() => {
  fetch();
});
